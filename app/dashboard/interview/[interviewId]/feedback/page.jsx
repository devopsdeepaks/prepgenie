...
const Feedback = ({ params }) => {
  ...
  const [rating, setRating] = useState(0); // Store overall rating
  ...
  const getFeedback = async () => {
    ...
    if (result.length > 0) {
      const totalRating = result.reduce((acc, item) => {
        const ratingValue = Number(item.rating); // Ensure it's a number
        if (isNaN(ratingValue)) {
          console.error("Invalid rating found:", item.rating); // Log if there's an issue
          return acc; // Skip invalid values
        }
        return acc + ratingValue;
      }, 0);

      const avgRating = (totalRating / result.length).toFixed(1); // Round to 1 decimal place
  ...
  return (
    <div className='min-h-screen dark flex flex-col p-10 pt-32'>
        ...
        <>
          ...
          <div className='bg-gradient-to-br from-[#0C0C0D] to-black border m-4 w-1/3 items-center justify-center p-4 rounded-lg flex gap-3'>
            ...
            <TextGenerateEffect
              ...
              words={`Your overall Rating: ${rating}`}
            />
          </div>
            ...
            {feedbackList.map((item, index) => (
              <Collapsible key={index} className='m-7'>
                <CollapsibleTrigger className='p-4 bg-cyan-500 text-white rounded-full text-xs md:text-sm text-center cursor-pointer'>
                  Ques #{index + 1}
                  <ChevronsUpDown className='gap-7 items-center h-5 w-5 justify-center inline' />
                </CollapsibleTrigger>
      ...
      <Button onClick={() => router.replace('/dashboard')} variant='outline' className='my-5 text-white'>Go Home</Button>
    </div>
  );
};
